...
def process_symbol(symbol, data, start_date, end_date, anchor_date, lookback_days, volume_threshold):
    try:
        if len(data) < 2:
            return None
...
def backtest_breakout_strategy(symbol, start_date, end_date, anchor_date, lookback_days=10, volume_threshold=1.5):
...
if st.button("Scan for AVWAP Crossings"):
    with st.spinner("Scanning..."):
        ...
        if not candidates.empty:
            ...
            st.info(f"No stocks found that crossed AVWAP within the last {lookback_days} days and have a closing price higher than AVWAP.")
...
if st.button("Run Backtest"):
    with st.spinner("Backtesting..."):
        data, breakout_dates = backtest_breakout_strategy(backtest_symbol, start_date, end_date, anchor_date, lookback_days, volume_threshold)
        if breakout_dates:
